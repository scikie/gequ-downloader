...
use crate::crawlers::{DownloadCrawler, HomepageCrawler, RankingCrawler, SearchCrawler};
use crate::database::Database;
use crate::models::{
    get_page_types, get_ranking_types, PageItem, PageSnapshot, RankingItem, Singer, Song,
};
...
async fn handle_crawl(cmd: CrawlCommands) -> Result<()> {
    ...
    match cmd {
        // 处理主页爬取
        // 【问题：参数file表意不明，指的是从文件爬取还是网络爬取。】
        CrawlCommands::Homepage {
            file,
            output,
            no_db,
        } => {
            ...
            let rows = vec![
                StatRow {
                    r#type: "最新搜索",
                    count: data.latest_searches.len(),
                },
                StatRow {
                    r#type: "热门关键词",
                    count: data.hot_keywords.len(),
                },
                StatRow {
                    r#type: "热门歌手",
                    count: data.hot_singers.len(),
                },
            ];
            // 【知识点：tabled库】
            // 自动将结构体数据格式化为ASCII表格
        ...
        CrawlCommands::Ranking {
            ...
            let ranking_name = ranking_types
                .get(ranking_type.as_str())
            ...
            if let (Some(s), Some(e)) = (start_page, end_page) {
                println!(
                    "{}",
                    format!("正在爬取 {} 第 {}-{} 页...", ranking_name, s, e).cyan()
                );
    ...
    match cmd {
        ...
        DbCommands::Stats => {
            ...
            let rows = vec![
                StatRow {
                    r#type: "歌手数",
                    count: stats["total_singers"].as_i64().unwrap_or(0),
                },
                StatRow {
                    r#type: "歌曲数",
                    count: stats["total_songs"].as_i64().unwrap_or(0),
                },
                StatRow {
                    r#type: "排行记录数",
                    count: stats["total_rankings"].as_i64().unwrap_or(0),
                },
                StatRow {
                    r#type: "搜索关键词数",
                    count: stats["total_keywords"].as_i64().unwrap_or(0),
                },
                StatRow {
                    r#type: "下载记录数",
                    count: stats["total_downloads"].as_i64().unwrap_or(0),
                },
                StatRow {
                    r#type: "页面快照数",
                    count: stats["total_page_snapshots"].as_i64().unwrap_or(0),
                },
                StatRow {
                    r#type: "页面条目数",
                    count: stats["total_page_items"].as_i64().unwrap_or(0),
                },
            ];
            let table = Table::new(rows).with(Style::rounded()).to_string();
            println!("{}", table);
            ...
            let rows: Vec<_> = rankings
                ...
                .map(|r| RankingRow {
                    ...
                    ranking_type: ranking_types
                        .get(r["ranking_type"].as_str().unwrap_or(""))
                        .map(|s| s.to_string())
                        .unwrap_or_else(|| r["ranking_type"].as_str().unwrap_or("").to_string()),
...
fn handle_stats(cmd: StatsCommands) -> Result<()> {
    ...
    match cmd {
        ...
        StatsCommands::TopSongs {
            ...
            let songs =
                db.get_top_appearing_songs(number, date_from.as_deref(), date_to.as_deref())?;
        ...
        StatsCommands::TopSingers {
            ...
            let (date_from, date_to) = parse_time_args(from, to, last)?;
            let singers =
                db.get_top_appearing_singers(number, date_from.as_deref(), date_to.as_deref())?;
        ...
        StatsCommands::History {
            ...
            let (date_from, date_to) = parse_time_args(from, to, last)?;
            let snapshots = db.get_page_snapshots(
                Some(&page_type),
                number,
                date_from.as_deref(),
                date_to.as_deref(),
    ...
    match cmd {
        ...
        ConfigCommands::Show => {
            ...
            let rows = vec![
                ConfigRow {
                    key: "cookie",
                    value: if config.cookie.is_empty() {
                        "(未设置)".to_string()
                    } else {
                        format!("{}...", &config.cookie[..20.min(config.cookie.len())])
                    },
                },
                ConfigRow {
                    key: "db_path",
